...
object task_collections {
  ...
  /**
    ...
    * *
    */
  def capitalizeIgnoringASCII(text: List[String]): List[String] = text.head :: text.tail.collect {
    case a if isASCIIString(a)  => a.toUpperCase()
    case b if !isASCIIString(b) => b.toLowerCase()
    case c                      => c
  }
  ...
  def numbersToNumericString(text: String): String = {
    ...
    text.foldLeft("")((z, curr) => z + m.getOrElse(curr, curr))
  }
  ...
  final case class Auto(mark: String, model: String)
  ...
  def filterAllLeftDealerAutoWithoutRight(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] =
    dealerTwo
      .foldLeft(dealerOne) { (acc, next) =>
        acc.filter(auto => auto != next)
      }
}
